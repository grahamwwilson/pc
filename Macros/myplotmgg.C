void myplotmgg( int irad=3, float ymax=8000.0, float ymin=0.0, string histtype="mggRCutHist")
{
...
// Create two TH1D's for the projections.

TH1D* hd = new TH1D("hd", "Radial bin;Di-photon mass (GeV)", 400, 0.0, 1.0);
TH1D* hm = new TH1D("hm", "Radial bin;Di-photon mass (GeV)", 400, 0.0, 1.0);

TH1D * hdpv = (TH1D*)fd->Get("numpvHist");
TH1D * hmpv = (TH1D*)fm->Get("numpvHist");

// Now loop over the 2d histograms filling the 1d histograms for 
// the appropriate radial bin

//int irad = 1;

for (int i=1; i<=400; i++){
    hd->Fill( (double(i)-0.5)/400.0, double(hd2d->GetBinContent(i,irad)) );
    hm->Fill( (double(i)-0.5)/400.0, double(hm2d->GetBinContent(i,irad)) );
} 
