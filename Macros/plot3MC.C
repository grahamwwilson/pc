void plot3MC(string histtype="r1dHist2", float ymax=700000.0, 
          float ymin=0.1, float xlmin = 0.33, float ylmin = 0.66, 
          bool logscale = false, string xtitle="Radius (cm)", string ytitle = "Conversion candidates per 0.1cm bin")
{
...
leg->AddEntry(hd,"2017 J/psi MC (All candidates)","l");
leg->AddEntry(hm,"2017 J/psi MC (Duplicates removed)","l");
...
c1->Print((histtype+"MC.pdf").c_str());
}
